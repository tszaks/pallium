...
func (s *Store) AcquireRepoLock(repoRoot, runID string, staleAfter time.Duration) (string, bool, error) {
	...
	return holder, ok, fmt.Errorf("acquire repo lock for %s (run %s) after %d attempts: %w", repoRoot, runID, repoLockBusyRetries, err)
}
...
func (s *Store) ReleaseRepoLock(repoRoot, runID string) error {
	...
	for attempt := 0; attempt <= repoLockBusyRetries; attempt++ {
		...
		if err == nil || !isSQLiteBusyErr(err) {
			...
			return fmt.Errorf("release repo lock for %s (run %s): %w", repoRoot, runID, err)
		}
		time.Sleep(10 * time.Millisecond)
	}
	// Exhausted retries under sustained contention: the row survives and
	// self-heals via the stale-takeover window (repoLockStaleAfter in
	// runtime.go) once this run's process exits, but callers need enough
	// context in the error to know which repo/run failed to release and act
	// on it (e.g. `pallium workflow` operators inspecting stderr) instead of
	// a bare driver error.
	return fmt.Errorf("release repo lock for %s (run %s) after %d attempts: %w", repoRoot, runID, repoLockBusyRetries, err)
}

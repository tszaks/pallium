...
func WorkflowTools() []ToolInfo {
	return []ToolInfo{
		...
		},
		{
			...
			Kind:        "control",
			Description: "Writes a narrator line to workflow stderr for progress and dropped-work transparency.",
			Returns:     "void",
			Example:     `log("verifying", findings.length, "findings after dedupe")`,
		},
		{
			Name:        "budget",
			Signature:   `budget.total; budget.spent(); budget.remaining()`,
			Kind:        "control",
			Description: "Tracks the local USD budget ceiling configured with --max-budget-usd.",
			Returns:     "total is null when no budget is configured; spent and remaining are numbers.",
			Example:     `if (budget.total !== null && budget.remaining() < 0.01) log("budget low");`,
			Notes:       []string{"Ultracode uses token budgets; Pallium maps the same shape onto local USD estimates via PALLIUM_WORKFLOW_AGENT_COST_USD."},
		},
		{
			Name:        "agent",
		...
		},
		{
			...
			Notes:       []string{"Default concurrency is 16 agents. Total agents per run default to 1000.", "Use as a barrier only when a later step needs all results together.", "Collection size is capped at 4096 items."},
		},
		{
			Name:        "pipeline",
			Signature:   `await pipeline(items, stage1, stage2, ...)`,
			Kind:        "control",
			Description: "Streams items through one or more stages, letting each item advance as soon as its prior stage completes.",
			Returns:     "Array of final-stage results in item order.",
			Example:     `await pipeline(files, file => agent("Inspect " + file), result => agent("Verify " + result.path))`,
			Notes:       []string{"Stages receive (prevResult, originalItem, index).", "A throwing stage drops that item to null and lets other items continue.", "Collection size is capped at 4096 items."},
		},
		{
			Name:        "check",

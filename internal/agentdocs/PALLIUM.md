...
6. **Knowledge base** — begin with `pallium knowledge context "task"` or MCP `pallium_knowledge_context`. The version-2 response is bounded to 12 KB and provides source paths, related tests, freshness and evidence state. Search returns five excerpts within 8 KB; get a section with `knowledge get <slug> --section Surface`. `--full` explicitly requests the legacy full payload. A current source map is not an audit: scanner-derived symbols are heuristic, citations establish identity, and audits assess bounded evidence without proving truth. Stale/legacy results are historical; refresh before relying on them. `knowledge build --no-model` is the free starting point. Background maintenance is opt-in per workspace with a global 40-attempt rolling daily cap. Only explicit `knowledge decisions link` imports authored decisions, which may conflict with implementation.

...
func TestBuildSkipsUnchangedModules(t *testing.T) {
	...
	if _, err := Build(store, repoID, repoRoot, BuildOptions{Synth: firstSynth, Only: []string{"storage"}}); err != nil {
		t.Fatalf("first build: %v", err)
	}

	secondSynth := &stubSynth{response: firstSynth.response}
	second, err := Build(store, repoID, repoRoot, BuildOptions{Synth: secondSynth, Only: []string{"storage"}})
	if err != nil {
...
func TestMaterializeWritesAndClearsStalePages(t *testing.T) {
	...
	if err := os.WriteFile(stale, []byte("<!-- Generated by `pallium knowledge build`. -->\n# old\n"), 0o644); err != nil {
...
func TestBuildFansOutWithoutLosingModules(t *testing.T) {
	...
	if report.Dropped != 3 {
		t.Fatalf("cross-module symbol claims should be rejected, got %d dropped", report.Dropped)
	}
	for _, doc := range docs {
		if doc.Slug == "storage" && !strings.Contains(doc.Body, "Open returns a Store") {
...
func TestAuditReportsMissingEvidenceInsteadOfSubstituting(t *testing.T) {
  ...
  {"index": 3, "ruling": "unclear", "why": "Not visible here."}
	...
	if strings.Contains(prompt, "Handle") {
		t.Fatalf("wrong-module claim survived citation validation:\n%s", prompt)
	}
...
func TestBuildRebuildsLegacyUncitedDocs(t *testing.T) {
	...
	if structural.Written != 1 {
		t.Fatalf("no-model build should replace legacy docs: %+v", structural)
	}

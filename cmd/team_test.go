...
// TestOpenTeamStoreUsesExplicitDBPathOverTestDBEnv verifies precedence: an
// explicit --db must always win over PALLIUM_TEST_DB. The safety net is only
// for the "forgot --db" case, never an override of deliberate intent.
func TestOpenTeamStoreUsesExplicitDBPathOverTestDBEnv(t *testing.T) {
	dir := t.TempDir()
	explicit := filepath.Join(dir, "explicit.sqlite")
	t.Setenv("PALLIUM_TEST_DB", filepath.Join(dir, "should-not-be-used.sqlite"))

	store, err := openTeamStore(explicit)
	if err != nil {
		t.Fatal(err)
	}
	defer store.Close()
...
// TestOpenTeamStoreRedirectsToTestDBWhenNoExplicitPathGiven is the
...
// PALLIUM_TEST_DB set, an empty --db now redirects there instead.
func TestOpenTeamStoreRedirectsToTestDBWhenNoExplicitPathGiven(t *testing.T) {
	dir := t.TempDir()
